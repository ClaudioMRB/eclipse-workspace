...
import dio.digitalinnovation.academiaDeGinastica.services.MatriculaService;
...
public ResponseEntity<MatriculaDto> findById(@PathVariable Long id) {
	MatriculaDto findbyid = service.findById(id);
  return ResponseEntity.ok(findbyid);
}

@GetMapping /*Busca paginada de clientes*/
public ResponseEntity<Page<MatriculaDto>> findAll(Pageable pageable) {
  Page<MatriculaDto> findAllDto = service.findAll(pageable);
  return ResponseEntity.ok(findAllDto);

}

@PostMapping /*Inserção de novo cliente*/
public ResponseEntity<MatriculaDto> insert(@RequestBody MatriculaDto insertDto) throws URISyntaxException {
...
public ResponseEntity<MatriculaDto> update(@PathVariable Long id, @RequestBody MatriculaDto updateDto) {
